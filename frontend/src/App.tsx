...
function ChallengesView({ items, total, category, onCategory, difficulty, onDifficulty, challengeSearch, onSearchChange, onOpen }: { items: ChallengeSummary[]; total: number; category: Filter; onCategory: (value: Filter) => void; difficulty: DifficultyFilter; onDifficulty: (value: DifficultyFilter) => void; challengeSearch: string; onSearchChange: (value: string) => void; onOpen: (item: ChallengeSummary) => void }) {
  ...
  if (categoryOptions.length > 4) return <ChallengeCatalog items={items} total={total} category={category} categoryOptions={categoryOptions} onCategory={onCategory} difficulty={difficulty} onDifficulty={onDifficulty} challengeSearch={challengeSearch} onSearchChange={onSearchChange} onOpen={onOpen} page={page} pageCount={pageCount} visibleItems={visibleItems} onPageChange={setPage} ko={ko} />
...
function ChallengeCatalog({ items, total, category, categoryOptions, onCategory, difficulty, onDifficulty, challengeSearch, onSearchChange, onOpen, page, pageCount, visibleItems, onPageChange, ko }: { items: ChallengeSummary[]; total: number; category: Filter; categoryOptions: Filter[]; onCategory: (value: Filter) => void; difficulty: DifficultyFilter; onDifficulty: (value: DifficultyFilter) => void; challengeSearch: string; onSearchChange: (value: string) => void; onOpen: (item: ChallengeSummary) => void; page: number; pageCount: number; visibleItems: ChallengeSummary[]; onPageChange: (page: number) => void; ko: boolean }) {
  return <ChallengeCatalogContent items={items} total={total} category={category} categoryOptions={categoryOptions} onCategory={onCategory} difficulty={difficulty} onDifficulty={onDifficulty} challengeSearch={challengeSearch} onSearchChange={onSearchChange} onOpen={onOpen} page={page} pageCount={pageCount} visibleItems={visibleItems} onPageChange={onPageChange} ko={ko} />
  return <div className="challenges-view"><PageIntro eyebrow="WARGAME" title={ko ? '다음 문제를 골라보세요.' : 'Pick your next challenge.'} description={ko ? '문제를 읽고, 단서를 따라 한 단계씩 풀어보세요.' : 'Read the brief, then follow the guide one step at a time.'} /><section className="challenge-toolbar"><div className="filter-tabs" aria-label={ko ? '카테고리' : 'Category'}>{categoryOptions.map((item) => <button key={item} className={category === item ? 'filter-tab active' : 'filter-tab'} type="button" onClick={() => onCategory(item)}>{item === 'ALL' ? (ko ? '전체 분야' : 'All fields') : item}</button>)}</div><div className="filter-tabs difficulty-tabs" aria-label={ko ? '난이도' : 'Difficulty'}>{(['ALL', 'BEGINNER', 'EASY', 'NORMAL', 'ADVANCED', 'EXPERT'] as DifficultyFilter[]).map((item) => <button key={item} className={difficulty === item ? 'filter-tab active diff-' + item.toLowerCase() : 'filter-tab diff-' + item.toLowerCase()} type="button" onClick={() => onDifficulty(item)}>{item === 'ALL' ? (ko ? '전체 난이도' : 'All levels') : difficultyLabel(item)}</button>)}</div></section><div className="challenge-count"><span>{ko ? `전체 ${total}개 중 조건에 맞는 ${items.length}개 문제` : `${items.length} matching challenges of ${total}`}</span><strong>{page} / {pageCount}</strong></div><div className="challenge-grid">{visibleItems.map((item) => <ChallengeCard key={item.id} item={item} onOpen={onOpen} />)}</div><nav className="challenge-pagination" aria-label={ko ? '문제 페이지 선택' : 'Challenge pages'}>{Array.from({ length: pageCount }, (_, index) => index + 1).map((pageNumber) => <button key={pageNumber} className={page === pageNumber ? 'page-number active' : 'page-number'} type="button" aria-current={page === pageNumber ? 'page' : undefined} aria-label={ko ? `${pageNumber}페이지` : `Page ${pageNumber}`} onClick={() => { onPageChange(pageNumber); window.scrollTo({ top: 0, behavior: 'smooth' }) }}>{pageNumber}</button>)}</nav>{items.length === 0 && <EmptyState />}</div>
}

function ChallengeCard({ item, onOpen }: { item: ChallengeSummary; onOpen: (item: ChallengeSummary) => void }) { const ko = document.documentElement.lang !== 'en'; return <article className={`challenge-card ${item.solved ? 'solved-card' : ''}`}><div className="card-top"><Badge tone={item.category}>{item.category}</Badge><Badge tone={item.difficulty}>{difficultyLabel(item.difficulty)}</Badge>{item.solved && <span className="solved-badge">✓ {ko ? '해결 완료' : 'Solved'}</span>}</div><h3>{item.title}</h3><p>{ko ? '문제 설명을 읽고 단서를 따라 FLAG를 찾아보세요.' : 'Read each brief and hunt for clues step by step. Submit the FLAG when you find it.'}</p><div className="card-bottom"><strong>{item.score}<small>pts</small></strong><span className="challenge-solve-count">★ {item.solveCount} {ko ? '명 해결' : 'solves'}</span>{item.solved && <span className="solved">{ko ? '해결 완료' : 'Solved'}</span>}<button className="card-open" type="button" onClick={() => onOpen(item)}>{item.solved ? (ko ? '다시 보기' : 'Review') : (ko ? '열기' : 'Open')}</button></div>{item.solved && <div className="solved-ribbon" aria-hidden="true">✓</div>}</article> }

function ChallengeCatalogContent({ items, total, category, categoryOptions, onCategory, difficulty, onDifficulty, challengeSearch, onSearchChange, onOpen, page, pageCount, visibleItems, onPageChange, ko }: { items: ChallengeSummary[]; total: number; category: Filter; categoryOptions: Filter[]; onCategory: (value: Filter) => void; difficulty: DifficultyFilter; onDifficulty: (value: DifficultyFilter) => void; challengeSearch: string; onSearchChange: (value: string) => void; onOpen: (item: ChallengeSummary) => void; page: number; pageCount: number; visibleItems: ChallengeSummary[]; onPageChange: (page: number) => void; ko: boolean }) {
  return <div className="challenges-view">
    <PageIntro eyebrow="WARGAME" title={ko ? '다음 문제를 골라보세요.' : 'Pick your next challenge.'} description={ko ? '문제를 읽고, 단서를 따라 한 단계씩 풀어보세요.' : 'Read the brief, then follow the guide one step at a time.'} />
    <section className="challenge-toolbar">
      <div className="filter-tabs" aria-label={ko ? '분야' : 'Category'}>{categoryOptions.map((item) => <button key={item} className={category === item ? 'filter-tab active' : 'filter-tab'} type="button" onClick={() => onCategory(item)}>{item === 'ALL' ? (ko ? '전체 분야' : 'All fields') : item}</button>)}</div>
      <div className="filter-tabs difficulty-tabs" aria-label={ko ? '난이도' : 'Difficulty'}>{(['ALL', 'BEGINNER', 'EASY', 'NORMAL', 'ADVANCED', 'EXPERT'] as DifficultyFilter[]).map((item) => <button key={item} className={difficulty === item ? `filter-tab active diff-${item.toLowerCase()}` : `filter-tab diff-${item.toLowerCase()}`} type="button" onClick={() => onDifficulty(item)}>{item === 'ALL' ? (ko ? '전체 난이도' : 'All levels') : difficultyLabel(item)}</button>)}</div>
    </section>
    <div className="challenge-count"><span>{ko ? `전체 ${total}개 중 조건에 맞는 ${items.length}개 문제` : `${items.length} matching challenges of ${total}`}</span><div className="challenge-count-right"><ChallengeSearch value={challengeSearch} onChange={onSearchChange} compact /><strong>{page} / {pageCount}</strong></div></div>
    <div className="challenge-grid">{visibleItems.map((item) => <ChallengeCard key={item.id} item={item} onOpen={onOpen} />)}</div>
    <nav className="challenge-pagination" aria-label={ko ? '문제 페이지 선택' : 'Challenge pages'}>{Array.from({ length: pageCount }, (_, index) => index + 1).map((pageNumber) => <button key={pageNumber} className={page === pageNumber ? 'page-number active' : 'page-number'} type="button" aria-current={page === pageNumber ? 'page' : undefined} aria-label={ko ? `${pageNumber}페이지` : `Page ${pageNumber}`} onClick={() => { onPageChange(pageNumber); window.scrollTo({ top: 0, behavior: 'smooth' }) }}>{pageNumber}</button>)}</nav>
...
function EnhancedRankingView({ rows, attendanceRows }: { rows: RankingRow[]; attendanceRows: AttendanceRankingRow[] }) {
  ...
  const podiumRows = visibleRows.filter((row) => row.rank <= 3).sort((left, right) => left.rank - right.rank)
  const tierGuide = [
