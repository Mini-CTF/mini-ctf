...
function PolicyView({ language, kind }: { language: Language; kind: PolicyKind }) {
  ...
  return <section className="policy-page"><div className="policy-card"><button className="back-link" type="button" onClick={() => navigate(-1)}>← {ko ? '이전 페이지' : 'Back'}</button><p className="eyebrow">{copy.eyebrow}</p><h1>{copy.title}</h1><p className="policy-intro">{copy.intro}</p><div className="policy-sections">{copy.sections.map(([heading, body]) => <article key={heading}><h2>{heading}</h2><p>{body}</p></article>)}</div></div></section>
}
