...
function VaultItemHoverPreview({ user, item, equipped }: { user: User; item: VaultCosmetic; equipped: VaultCosmetic[] }) {
  ...
  if (item.type === 'CREDIT') return <div className="vault-hover-preview vault-hover-credit" aria-hidden="true"><strong>힌트 크레딧</strong><span>막힌 문제에서 힌트를 열 수 있어요.</span></div>
  return <div className="vault-hover-preview" aria-hidden="true"><span className={`vault-preview-avatar ${frame ? `equipped-${frame}` : ''}`}>{(user.nickname || user.username).slice(0, 2).toUpperCase()}</span><strong>{user.nickname || user.username}{accessory && <i className="profile-accessory">◈</i>}</strong>{title && <span className={`profile-title vault-profile-title ${titleTone(title)}`}>{cosmeticLabel(title)}</span>}<small>착용 미리보기</small></div>
}
