...
function LegacyLoginView({ onBack, onAuth }: { onBack: () => void; onAuth: (result: { token: string; user: User }) => void }) {
  ...
  return <div className="auth-page"><button className="back-link" type="button" onClick={onBack}>← Back home</button><div className="auth-card"><p className="eyebrow">SECURE ACCESS</p><h1>{registering ? 'Create your account.' : 'Sign in to continue.'}</h1><form className="auth-form" onSubmit={submit}><label>Username<input name="username" placeholder="e.g. flagbox_user" required minLength={3} maxLength={50} pattern="[A-Za-z0-9_]+" title="Use only letters, numbers, and underscores." autoComplete="username" /></label>{registering && <label>Display name (optional)<input name="nickname" placeholder="e.g. FlagBox Learner" maxLength={80} /></label>}<label>Password<input name="password" type="password" placeholder="e.g. 8 or more characters" required minLength={registering ? 8 : undefined} maxLength={100} autoComplete={registering ? 'new-password' : 'current-password'} /></label>{registering && <label>Confirm password<input name="passwordConfirmation" type="password" placeholder="Enter the same password again" required minLength={8} maxLength={100} autoComplete="new-password" /></label>}<button className="button primary" type="submit">{registering ? 'Create account' : 'Sign in'}</button></form>{error && <p className="alert error">{error}</p>}{providers.length > 0 && <><div className="auth-divider"><span>or continue with</span></div><div className="social-buttons">{providers.map((provider) => <button className={`social-button oauth-${provider}`} type="button" key={provider} onClick={() => { window.location.href = `${oauthBaseUrl}/api/auth/oauth/${provider}/authorize` }}><ProviderIcon provider={provider} /><span>Continue with {provider[0].toUpperCase() + provider.slice(1)}</span></button>)}</div></>}<p className="auth-footnote">{registering ? 'Already have an account?' : 'New to Mini CTF?'} <button type="button" onClick={() => setRegistering(!registering)}>{registering ? 'Sign in' : 'Create an account'}</button></p></div></div>
}

function LoginView({ onBack, onAuth }: { onBack: () => void; onAuth: (result: { token: string; user: User }) => void }) {
  return <div className="auth-login-shell"><section className="auth-showcase" aria-hidden="true"><Beams className="auth-showcase__beams" beamWidth={2} beamHeight={42} beamNumber={18} rotation={90} /><div className="auth-showcase__content"><span className="auth-showcase__mark">FlagBox</span><p>LEARN · ANALYZE · CAPTURE</p></div></section><LegacyLoginView onBack={onBack} onAuth={onAuth} /></div>
}
