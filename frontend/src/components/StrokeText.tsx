...
export default function StrokeText({ text, strokeColor = '#ffffff', fillColor = '#ffffff', strokeWidth = 1.25, drawDuration = 1.55, fillDelay = 0.18, fillDuration, stagger = 0.055, fontSize = 220, fontWeight = 800, letterSpacing = 0, characterOffsets, onFillComplete, className = '' }: StrokeTextProps) {
  ...
  useEffect(() => {
    ...
    const resolvedFillDuration = fillDuration ?? Math.max(0.4, drawDuration * 0.5)
    if (wipe) timeline.to(wipe, { attr: { width: box.width }, duration: resolvedFillDuration, ease: 'power2.inOut' }, drawDuration + fillDelay)
    if (onFillComplete) timeline.call(onFillComplete, [], drawDuration + fillDelay + resolvedFillDuration)
    return () => { timeline.kill(); gsap.killTweensOf(targets) }
  }, [box, dash, drawDuration, fillDelay, fillDuration, onFillComplete, stagger])
