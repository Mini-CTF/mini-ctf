...
이 프로젝트는 사용자 1명과 팀원 1명이 함께 개발한다. 이전의 개인 바이브코딩 전제보다 아래 역할 규칙을 우선한다.

## 담당자

- 사용자: 백엔드 담당. Java 21/Spring Boot 공개 REST API, FastAPI 내부 REST 서비스, PostgreSQL, Flyway, 인증·권한·Challenge·FLAG·점수·랭킹·관리자 API를 담당한다.
- 팀원: 프론트엔드 담당. React/TypeScript, Vite, 라우팅, API client, 상태 관리, 화면·컴포넌트·반응형·접근성을 담당한다.
- 공동: REST API 계약, 보안 요구사항, 테스트, Docker, 문서, Git/GitHub 통합을 함께 검토한다.

## 역할 전환 트리거

사용자가 `나는 백엔드 담당이야`라고 말하면 Codex는 백엔드 모드로 전환한다.

1. 먼저 `AGENTS.md`, `README.md`, `DESIGN.md`를 읽는다.
2. 기존 `backend/`, FastAPI 서비스 디렉터리, DB 마이그레이션, 설정 파일, 테스트, API 문서를 읽는다.
3. 기존 백엔드 구현과 REST API 계약을 재사용하고, 프론트엔드 파일은 계약 확인에 필요한 범위만 읽는다.
4. Java/Spring Boot는 업무 데이터와 PostgreSQL 트랜잭션의 기준 서버로 유지한다.
5. FastAPI는 내부 보조 REST 서비스로만 사용하며, 인증·점수·Solve·DB 스키마의 중복 소유자를 만들지 않는다.

사용자가 `나는 프론트엔드 담당이야`라고 말하면 Codex는 프론트엔드 모드로 전환한다.

1. 먼저 공통 문서와 기존 `frontend/` 전체 구조를 읽는다.
2. API client, 타입 정의, 라우팅, 기존 React 컴포넌트·페이지·스타일을 우선 확인한다.
3. `DESIGN.md`의 현재 색상과 스타일을 유지하고 기존 토큰·컴포넌트를 우선 재사용한다.
4. 백엔드는 API 요청·응답 계약 확인에 필요한 범위만 읽고, 서버 로직은 임의로 수정하지 않는다.
5. API 계약 변경이 필요하면 백엔드 담당자에게 전달할 변경안을 먼저 문서화한다.

역할 문장이 없거나 두 영역을 동시에 바꾸는 요청이면 Codex는 작업 전에 담당 범위와 변경 파일을 짧게 보고한다. 역할 문장은 단순한 설명이 아니라 해당 세션에서 우선적으로 읽을 작업 영역을 결정하는 신호다.
...
담당자는 자신의 브랜치에서 작업하고, API 계약·DB 변경·공통 문서 변경은 Pull Request 설명에 영향 범위를 적는다. `main`에 병합하기 전에 담당 영역 테스트와 통합 확인을 실행한다. 강제 push와 파괴적인 Git 명령은 사용자의 명시적 요청 없이는 실행하지 않는다.

## Codex 컨텍스트 관리

- 컨텍스트 사용량이 40% 이상이면, 해당 실행 환경에서 지원하는 경우 별도 허가 없이 `/compact`를 실행해 대화를 간소화한 뒤 현재 작업을 이어서 진행한다.
- `/compact` 명령을 직접 실행할 수 없고 컨텍스트가 자동 축약되는 환경에서는 자동 축약 결과를 이어받아 완료된 작업을 반복하지 않고 계속 진행한다.
