...
final class FlagboxChallengeCatalog {
                    ...
                    flag ->
                        txt(
                            "print(\"\".join(chr(ord(c) - 1) for c in s))\ns = \""
                                + new String(
                                    flag.chars().map(c -> c + 1).toArray(), 0, flag.length())
                                + "\"\n")),
  ...
  private static void addExpandedSeeds(List<Seed> seeds, String category, String prefix) {
    String[] difficulties = {"BEGINNER", "EASY", "NORMAL", "ADVANCED", "EXPERT"};
  ...
  private static String categoryTopic(String category, int number) {
    String[] web = {
      "공개 소스의 메모", "요청 기록의 단서", "브라우저 저장소", "응답 헤더", "배포 파일 점검", "URL 매개변수", "쿠키 흔적", "프런트 코드 읽기"
    };
    String[] forensic = {
      "사진 파일의 꼬리", "로그의 시간표", "메모리 조각", "문서 메타데이터", "네트워크 캡처", "압축 파일의 흔적", "삭제된 메모", "파일 시그니처"
    };
    String[] reversing = {
