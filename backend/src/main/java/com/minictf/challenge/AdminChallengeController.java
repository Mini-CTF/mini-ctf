...
public class AdminChallengeController {
  ...
  @PostMapping
  public ResponseEntity<ApiResponse<ChallengeDtos.AdminView>> create(
      @Valid @RequestBody ChallengeDtos.AdminRequest r) {
    return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.ok(service.create(r)));
  }

  @PutMapping("/{id}")
  public ApiResponse<ChallengeDtos.AdminView> update(
      @PathVariable Long id, @Valid @RequestBody ChallengeDtos.AdminRequest r) {
  ...
  @PostMapping(value = "/{id}/artifact", consumes = "multipart/form-data")
  public ApiResponse<ChallengeDtos.ArtifactView> uploadArtifact(
      @PathVariable Long id, @RequestPart("file") MultipartFile file) {
    return ApiResponse.ok(service.uploadArtifact(id, file));
  }
