...
public interface SolveRepository extends JpaRepository<Solve,Long>{
    ...
    @Query("select s from Solve s join fetch s.challenge where s.user.id=:userId order by s.solvedAt desc") List<Solve> findByUserId(@Param("userId")Long userId);
    ...
    @Query("select s.challenge.id from Solve s where s.user.id=:userId") Set<Long> findChallengeIdsByUserId(@Param("userId")Long userId);
}
