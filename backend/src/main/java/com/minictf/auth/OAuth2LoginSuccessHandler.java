...
public class OAuth2LoginSuccessHandler implements AuthenticationSuccessHandler {
    ...
    @Override @Transactional public void onAuthenticationSuccess(HttpServletRequest request,HttpServletResponse response,Authentication authentication) throws IOException,ServletException {
        ...
        String subject=providerValue(principal,provider,"id"); if(subject==null||subject.isBlank()){response.sendError(400,"OAuth subject is missing");return;}
        ...
        response.sendRedirect(redirect+"#token="+URLEncoder.encode(jwt.createToken(user.getId(),user.getRole()),StandardCharsets.UTF_8));
    }
    private User create(String provider,String subject,OAuth2User principal){
        String username=provider+"_"+hash(subject); User user=new User();user.setUsername(username);user.setNickname(first(providerValue(principal,provider,"nickname"),username));user.setRole("USER");user.setScore(0);user=users.save(user);
    ...
    private static String nested(OAuth2User user,String parent,String...keys){Object raw=user.getAttributes().get(parent);if(!(raw instanceof java.util.Map<?,?> map))return null;for(String key:keys){Object value=map.get(key);if(value instanceof java.util.Map<?,?> child){Object nickname=child.get("nickname");if(nickname!=null)return nickname.toString();}if(value!=null&&!value.toString().isBlank())return value.toString();}return null;}
    private static String providerValue(OAuth2User user,String provider,String purpose){if("id".equals(purpose)){if("google".equals(provider))return attribute(user,"sub");if("naver".equals(provider))return nested(user,"response","id");return attribute(user,"id");}if("naver".equals(provider))return first(nested(user,"response","nickname","name","email"),attribute(user,"name","email"));if("kakao".equals(provider))return first(nested(user,"properties","nickname"),nested(user,"kakao_account","profile"));return attribute(user,"name","login","nickname","email");}
