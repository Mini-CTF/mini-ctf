...
public class AuthService {
    ...
    @Transactional public AuthDtos.AuthResponse register(AuthDtos.RegisterRequest request) {
        ...
        User user = new User(); user.setUsername(username); user.setNickname(request.nickname()==null || request.nickname().isBlank()?username:request.nickname().trim());
    ...
    @Transactional(readOnly=true) public AuthDtos.AuthResponse login(AuthDtos.LoginRequest request) {
        ...
        if (user.getPasswordHash()==null || !encoder.matches(request.password(), user.getPasswordHash())) throw new InvalidCredentialsException();
        return issue(user);
    }
    public AuthDtos.AuthResponse issue(User user) { return new AuthDtos.AuthResponse(jwt.createToken(user.getId(), user.getRole()), toView(user)); }
